...
export function PreviewPanel() {
  ...
  const availableModes: PreviewMode[] = ['markdown', 'html', 'local', 'pdf', 'collabora'];
  ...
  }, [markdown, config, fitWordPreview, meta, previewMode, previewContainer, t]);
  ...
  }, [availableModes, capabilities.collabora, capabilities.pdfLocal, previewMode, setPreviewMode]);
      ...
      return (
        <div key="preview-local" className="flex-1 overflow-auto bg-gray-200 p-5">
          <div
            ...
            style={{ zoom: wordZoom as any, width: 'max-content', minWidth: '100%', margin: '0 auto' }}
          />
        </div>
      );
    }
    ...
    <div className="flex-1 flex flex-col min-w-[380px] bg-gray-200">
      <div className="bg-white border-b border-gray-200 px-3 py-1.5 text-xs font-semibold text-gray-600 flex items-center shrink-0 flex-wrap gap-2">
        ...
        <select value={previewMode} onChange={e => setPreviewMode(e.target.value as PreviewMode)} className="ml-2 text-[11px] py-0.5 px-1 rounded border border-gray-300 bg-white cursor-pointer">
          ...
          <option value="pdf" disabled={!capabilities.pdfLocal}>{t('modePdf')}</option>
          <option value="collabora" disabled={!capabilities.collabora}>{t('modeCollabora')}</option>
        </select>
      ...
      <div className="bg-white border-b border-gray-200 px-2.5 py-1.5 flex gap-1.5 items-center shrink-0 flex-wrap">
        ...
        {previewMode === 'local' && (
          <>
            <button onClick={() => setWordZoom((v) => Math.max(0.35, Number((v - 0.1).toFixed(2))))} className="px-2 py-1 border border-gray-200 rounded text-xs hover:bg-gray-100">-</button>
            <span className="text-[11px] text-gray-600 w-12 text-center">{Math.round(wordZoom * 100)}%</span>
            <button onClick={() => setWordZoom((v) => Math.min(1.6, Number((v + 0.1).toFixed(2))))} className="px-2 py-1 border border-gray-200 rounded text-xs hover:bg-gray-100">+</button>
            <button onClick={() => setWordZoom(1)} className="px-2 py-1 border border-gray-200 rounded text-xs hover:bg-gray-100">100%</button>
            <button onClick={fitWordPreview} className="px-2 py-1 border border-gray-200 rounded text-xs hover:bg-gray-100">适配宽度</button>
          </>
        )}
        ...
        <button onClick={handleExportPdf} disabled={!capabilities.pdfLocal} className="px-2 py-1 border border-gray-200 rounded text-xs hover:bg-gray-100 disabled:opacity-50" title={!capabilities.pdfLocal ? '该功能需要服务端版，GitHub Pages 纯前端版不可用' : ''}>
          {t('exportPdf')}
        </button>
        ...
        {!capabilities.pdfLocal || !capabilities.collabora ? (
          <span className="text-[11px] text-amber-600">
            该功能需要服务端版，GitHub Pages 纯前端版不可用
          </span>
        ) : null}
